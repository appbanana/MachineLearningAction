...
def load_dataset(file_name):
    ...
    with open(file_name, 'r') as fr:
        for line in fr.readlines():
            ...
            label_mat.append(int(float(line_arr[2])))
...
def innerL(i, os):
    ...
    if ((os.label_mat[i] * ei < -os.tol) and (os.alphas[i] < os.C)) or (
        ...
        update_ek(os, i)
        # 第七步：计算b1， b2
        # b1 = os.b - ei - os.label_mat[i] * (os.alphas[i] - alpha_i_old) * os.X[i, :] * os.X[i, :].T - os.label_mat[
        #     j] * (os.alphas[j] - alpha_j_old) * os.X[i, :] * os.X[j, :].T
        # b2 = os.b - ej - os.label_mat[i] * (os.alphas[i] - alpha_i_old) * os.X[i, :] * os.X[j, :].T - os.label_mat[
        #     j] * (os.alphas[j] - alpha_j_old) * os.X[j, :] * os.X[j, :].T
        # 加入核函数后
...
def smoP(data_mat, class_labels, C, toler, max_iter, k_tup=('lin', 0)):
    os = OptStruct(np.mat(data_mat), np.mat(class_labels).transpose(), C, toler, k_tup)
    iter_num = 0
    entire_set = True
    alpha_pairs_changed = 0
