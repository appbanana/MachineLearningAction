...
def adaboost_train_ds(data_mat, class_labels, max_iter=40):
    ...
    for i in range(max_iter):
        best_stump, min_error, predict_value, = build_stump(data_mat, class_labels, D)
        # print('D:', D.T)
        # 计算alpha值
...
if __name__ == '__main__':
    # 加载简单的数据
    train_data, train_label = load_data('./data/horseColicTraining2.txt')
    # 获取弱分类器
    classifier_arr = adaboost_train_ds(train_data, train_label, 10)
    print(classifier_arr)

    test_data, test_label = load_data('./data/horseColicTest2.txt')
    predict_labels = ada_classify(test_data, classifier_arr)
    m = np.shape(test_data)[0]
    # 初始化为元素1的矩阵
    result_arr = np.mat(np.ones((m, 1)))
    # 将正确的标签置位1，错误的标签置位0 便于统计正确率
    result_arr[predict_labels != test_label.reshape(predict_labels.shape)] = 0
    # 51.0
    correct_rate = 1.0 * result_arr.sum() / m
    print(result_arr.sum())
    # 正确率： 0.7611940298507462
